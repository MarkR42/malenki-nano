...
void blinky_loop() {
    ...
    // Which can be 8 centiseconds (or about 1/12 sec)
    uint32_t blinkticks = get_tickcount() / 8; // Compiler should optimise to a shift
    // Find out how far through the period we are?
    uint8_t blinkticks1 = ((uint8_t) blinkticks) & 0x1f; 
    bool red_on = ((blinkticks1 / 4) < blinky_state.flash_count) && ((blinkticks1 & 3) == 0);
